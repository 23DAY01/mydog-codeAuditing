...
export default defineComponent({
  ...
  methods: {
    ...
    async setPicPath(str){
      this.$g.locatemenus.pic_path=str
    },
    ...
    handleUpload () {
      ...
      request.post('http://127.0.0.1:5000/upload', {
        ...
        .then((response) => {
          ...
          if(response[1].result == undefined) {
              message.warn({
                content: "分析数据异常",
                style: {
                  marginTop: '20vh',
          ...
          this.setPicPath(response[0]);
          this.setAndJumpToLocation(response[1].result);
        })
        .catch((error) => {
